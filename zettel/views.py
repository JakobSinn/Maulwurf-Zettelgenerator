...
class UniqueGremiumListView(ListView):
    ...
    def get_queryset(self):
        try:
            return (
                Kandidatur.objects.filter(bestaetigt="ja")
                .filter(gewaehlt="noch nicht")
                .values("gremium")
                .distinct()
            )
        except:  # noqa: E722
...
def KandiView(request, gremiumwanted=""):
    if request.method == "POST":
        ...
        if not (
            stimmzettel_eigenschaften_form.is_valid() and kandidaten_form.is_valid()
        ):
        ...
        kandidaten = [
            nd.cleaned_data
            for nd in kandidaten_form
            if nd.cleaned_data.get("first_name")
        ]

        kandidaten_namen = list(
            map(
                lambda kandidat: kandidat["first_name"] + " " + kandidat["last_name"],
                kandidaten,
            )
        )

        kandidaten_kommentare = list(
            map(lambda kandidat: kandidat["candidate_comment"], kandidaten)
        )

        # Wenn keine Kampfkandidatur vorliegt, wird bei allen Kandidaten mit ja / nein / enth. abgestimmt.
        is_jnewahl = not (
            len(kandidaten_namen) > stimmzettel_eigenschaften["anzahl_stimmen"]
        )
        ...
        if "willstimmzettel" in request.POST:
            ...
            try:
                response = HttpResponse(
                    zetteldrucken(
                        kandidaten_namen,
                        stimmzettel_eigenschaften["gremien_name"],
                        abstimmungsdatum=stimmzettel_eigenschaften["abstimmung_datum"],
                        extras=kandidaten_kommentare,
                        stimmen=stimmzettel_eigenschaften["anzahl_stimmen"],
                        jne=is_jnewahl,
                        printbel=stimmzettel_eigenschaften["belehrung_drucken"],
                    ),
        ...
        if "willbericht" in request.POST:
            ...
            try:
                response = HttpResponse(
                    bericht(
                        kandidaten=kandidaten_namen,
                        posten=stimmzettel_eigenschaften["gremien_name"],
                        jne=is_jnewahl,
                        abstimmungsdatum=stimmzettel_eigenschaften["abstimmung_datum"],
                    ),
                    content_type="application/pdf",
        ...
        stimmzettel_eigenschaften_form = StimmzettelEigenschaftenForm(
            initial=initialdata
        )
        if gremiumwanted:
